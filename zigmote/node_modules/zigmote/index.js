...
function startRoom(data) {
	...
	if (!rooms.hasOwnProperty(roomid)) {
		rooms[roomid] = {
			...
			users : {},
			userlist : synclist.create(roomid + ":userlist"),
		};

		rooms[roomid].roomdata.add(data.roomname, "roomname");
		rooms[roomid].roomdata.add(rooms[roomid].userlist.id, "userslist");
		rooms[roomid].userlist.add("Test user", 123);
...
function joinRoom(socket, roomid, data) {
	...
	joining.userlist.add(username, socket.userid);
...
function leaveRoom(socket) {
	if (undefined !== socket.roomid) {
		rooms[socket.roomid].userlist.remove(socket.userid);
		delete rooms[socket.roomid].users[socket.userid];
		// TODO: Release any locks held by this user
	}
}
...
function zigmotify(socket) {
	...
	socket.on('zigmote-startRoom', function(data) {
		...
		socket.emit('zigmote-roomStarted', {
			roomid : roomid,
			roomdata : rooms[roomid].roomdata.id,
		});

		// TODO : sync this with changes
	...
	socket.on('zigmote-joinRoom', function(data) {
		...
		socket.on("zigmote-rename", function(data) {
			rooms[socket.roomid].userlist.set(socket.userid, data.username);
