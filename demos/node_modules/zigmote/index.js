...
function startRoom(socket, data) {
	...
	// maintain our rooms synclist
	synclist.create("roomslist:"+data.roomtype).set(roomid, {roomname : data.roomname});
...
function doUnlock(socket, lockid) {
	// sanity - 
	// TODO: Look into this a bit more, why is it happening?
	if (!rooms.hasOwnProperty(socket.roomid)) {
		return;
	}
...
function zigmotify(socket) {
	...
	socket.on('zigmote-startRoom', function(data) {
		...
		socket.on('zigmote-rename', function(data) {
			...
			var list = synclist.create("roomslist:"+socket.roomtype);
			list.set(roomid, {roomname : data.roomname});
		});
		...
		socket.on('disconnect', function () {
			...
			if (!rooms.hasOwnProperty(socket.roomid)) return;

			rooms[socket.roomid].cleanupTimer = setTimeout((function(rid){ return function(){
	...
	socket.on('zigmote-joinRoom', function(data) {
		...
		socket.emit('zigmote-joinedRoom', { 
			...
			roomshareddata : rooms[socket.roomid].roomdata.data,
			userslistid : rooms[socket.roomid].userslist.id,
		});	
	});
}
