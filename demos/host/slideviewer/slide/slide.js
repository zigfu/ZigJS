slideshow = (function() {
  ...
  SlideShow.prototype = {
    ...
    first: function() {
      var first = query('.slide:nth-child(1)');
      this._update((first) ? first.id : this.current);
    },
    last:function() {
      var last = query('.slide:nth-child('+ this._slides.length +')');
      this._update((last) ? last.id : this.current);
    },
  ...
  if (false) {
    ...
    var linkEls = queryAll('link.theme');
    var stylesheetPath = sessionStorage['theme'] || 'slide/default.css';
