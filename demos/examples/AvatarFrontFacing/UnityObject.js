...
var unityObject = function() {
	...
	function createUnity(id, attributes, params, callback) {
		...
		if (!re) {
			if (callback) {
				callback({ success : false, id : id, type : lastType, error : "object not found" });
			}
			return;
		}
		//if (ua.win && ua.ie) {
		if (true) {
			...
			console.log("Inner: " + re.innerHTML);
			re.innerHTML = '<object type="' + pluginMimeType + '" style="display: block; width: ' + appendPX(attributes["width"]) + '; height: ' + appendPX(attributes["height"]) + ';"' + at + '>' + pt + '</object>';
			console.log("Inner after: " + re.innerHTML);
		...
		getObjectById(id, function(obj) {
			if (obj) {
				...
				//removeUnity(id);
				console.log("should have removed unity");
			}
			if (callback) {
				var error = "";
				if (!obj)
		...
		if (re) {
			...
			if (enableJava && ua.java && !triedJava && !getCookie(triedJavaCookie)) {
				...
				if (autoInstall) {
					...
					imagesToWaitFor.push(getUnityImgID);
					if (enableJVMPreloading)
						preloadJava = true;
				}
			}
			...
			onclick += "unityObject.notifyBeginInstall('"+id+"','"+type+"');";
			var bg = getColor(params, "backgroundcolor");
			var tc = getColor(params, "textcolor");
			var bc = getColor(params, "bordercolor");
			if (ua.win && ua.ie) {
				var it = '<img id="' + getUnityImgID + '" alt="' + msg + '" src="' + img + '" width="' + imgWidth + '" height="' + imgHeight + '" style="border-width: 0px;" />';
				...
				ie.setAttribute("id", getUnityImgID);
	...
	function preloadJVMWhenReady() {
		var needToWait = false;
		for (i = 0; i < imagesToWaitFor.length; i++) {
			if (imagesToWaitFor[i] == null)
				continue;
	...
	function jvmPreloaded(id) {
		// can be used to react on JVM start event
	}
	...
	function javaInstallDone(id, success, error) {
		if ( success == false ) {
			applet = applets[id];
			createMissingUnity(id, applet.attributes, applet.params, applet.callback, applet.broken, "java", error);
		}
	}
	...
	return {
				...
				addDomLoadEvent(function() {
					...
					detectUnity(function(status) {
						if (status == "installed") {
							...
							createMissingUnity(id, clonedAttributes, clonedParams, callback, status == "broken", null, "Not installed");
		...
		notifyBeginInstall : function(id, type) {
			setTimeout("unityObject.notifyBeginInstallDirect(\"" + id + "\", \"" + type + "\");", 0);
		},

		// private function
		notifyBeginInstallDirect : function(id, type) {
